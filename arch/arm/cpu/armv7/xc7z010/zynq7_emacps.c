...
void XEmacPs_Start(XEmacPs *sprt_emacps)
{
    ...
    if ((sprt_emacps->Options & (u32)XEMACPS_TRANSMITTER_ENABLE_OPTION)!=0x00000000U) 
    {
    ...
    XEmacPs_IntEnable(sprt_emacps, 
            (XEMACPS_IXR_TX_ERR_MASK | XEMACPS_IXR_RX_ERR_MASK | (u32)XEMACPS_IXR_FRAMERX_MASK | (u32)XEMACPS_IXR_TXCOMPL_MASK));
...
void XEmacPs_SetQueuePtr(XEmacPs *sprt_emacps, kuint32_t QPtr, u8 QueueNum, u16 Direction)
{
    /*!< Assert bad arguments and conditions */
    if ((!sprt_emacps) ||
        (!sprt_emacps->IsReady))
...
kint32_t XEmacPs_SetMacAddress(XEmacPs *sprt_emacps, void *AddressPtr, u8 Index)
{
    u32 MacAddr;
    ...
    if ((!sprt_emacps) ||
        (!Aptr) ||
        (!sprt_emacps->IsReady) ||
        ((IndexLoc <= (u8)XEMACPS_MAX_MAC_ADDR) && (IndexLoc > 0x00U)))
        return -ER_NREADY;
...
kint32_t XEmacPs_SetTypeIdCheck(XEmacPs *sprt_emacps, u32 Id_Check, u8 Index)
{
    u8 IndexLoc = Index;

    if ((!sprt_emacps) ||
        (!sprt_emacps->IsReady) ||
        ((IndexLoc <= (u8)XEMACPS_MAX_TYPE_ID) && (IndexLoc > 0x00U)))
        return -ER_NREADY;
...
kint32_t XEmacPs_SetOptions(XEmacPs *sprt_emacps, u32 Options)
{
...
kint32_t XEmacPs_ClearOptions(XEmacPs *sprt_emacps, u32 Options)
{
    u32 Reg;		                                /*!< Generic */
    u32 RegNetCfg;		                            /*!< Reflects original contents of NET_CONFIG */
    u32 RegNewNetCfg;	                            /*!< Reflects new contents of NET_CONFIG */
...
void XEmacPs_Reset(XEmacPs *sprt_emacps)
{
    ...
    Reg |= ((u32)XEMACPS_NWCFG_100_MASK | (u32)XEMACPS_NWCFG_FDEN_MASK | (u32)XEMACPS_NWCFG_UCASTHASHEN_MASK);
    ...
    XEmacPs_SetQueuePtr(sprt_emacps, 0, 0x00U, (u16)XEMACPS_SEND);
    if (sprt_emacps->Version > 2)
        XEmacPs_SetQueuePtr(sprt_emacps, 0, 0x01U, (u16)XEMACPS_SEND);
    XEmacPs_SetQueuePtr(sprt_emacps, 0, 0x00U, (u16)XEMACPS_RECV);
...
/*!< -------------------------------------------------------------------------------- */
/*!< -------------------------------------------------------------------------------- */
kint32_t XEmacPs_PhyRead(XEmacPs *sprt_emacps, u32 PhyAddress, u32 RegisterNum, u16 *PhyDataPtr)
{
    u32 Mgtcr;
    volatile u32 Ipisr;
    kuint32_t Status;
    ...
    /*!< Read data */
    *PhyDataPtr = (u16)XEmacPs_ReadReg(sprt_emacps->Config.BaseAddress, XEMACPS_PHYMNTNC_OFFSET);

    return ER_NORMAL;
}

kint32_t XEmacPs_PhyWrite(XEmacPs *sprt_emacps, u32 PhyAddress, u32 RegisterNum, u16 PhyData)
{
    u32 Mgtcr;
    volatile u32 Ipisr;
    kuint32_t Status;
...
void XEmacPs_SetOperatingSpeed(XEmacPs *sprt_emacps, u16 Speed)
{
    u32 Reg;
...
static void XEmacPs_SetUpSLCRDivisors(kuint32_t mac_baseaddr, s32_t speed)
{
...
kint32_t XEmacPs_BdRingClone(XEmacPs_BdRing *RingPtr, XEmacPs_Bd *SrcBdPtr, u8 Direction)
{
    u32 i;
    kuint32_t CurBd;
...
kint32_t XEmacPs_BdRingAlloc(XEmacPs_BdRing *RingPtr, u32 NumBd, XEmacPs_Bd **BdSetPtr)
{
...
kint32_t XEmacPs_BdRingUnAlloc(XEmacPs_BdRing * RingPtr, u32 NumBd, XEmacPs_Bd * BdSetPtr)
{
    if ((!RingPtr) ||
        (!BdSetPtr))
        return -ER_UNVALID;
...
kint32_t XEmacPs_BdRingFree(XEmacPs_BdRing * RingPtr, u32 NumBd, XEmacPs_Bd * BdSetPtr)
{
...
kuint32_t XEmacPs_BdRingFromHwTx(XEmacPs_BdRing * RingPtr, u32 BdLimit, XEmacPs_Bd ** BdSetPtr)
{
    XEmacPs_Bd *CurBdPtr;
    u32 BdStr = 0U;
    u32 BdCount;
...
kuint32_t XEmacPs_BdRingFromHwRx(XEmacPs_BdRing * RingPtr, u32 BdLimit, XEmacPs_Bd ** BdSetPtr)
{
    XEmacPs_Bd *CurBdPtr;
    u32 BdStr = 0U;
    u32 BdCount;
...
kint32_t XEmacPs_BdRingToHw(XEmacPs_BdRing * RingPtr, u32 NumBd, XEmacPs_Bd * BdSetPtr)
{
    XEmacPs_Bd *CurBdPtr;
    u32 i;
...
//  struct xtopology_t *xtopologyp = &xtopology[xemac->topology_index];
    ...
    index = 0;
    gigeversion = ((XEmacPs_ReadReg(xemacpsif->emacps.Config.BaseAddress, 0xFC)) >> 16) & 0xFFF;
...
//		Xil_SetTlbAttributes((s32_t)bd_space, DEVICE_MEMORY); // addr, attr
    ...
    if (gigeversion > 2)
    {
        ...
        XEmacPs_WriteReg(xemacpsif->emacps.Config.BaseAddress, XEMACPS_RXQ1BASE_OFFSET, (kuint32_t)bdrxterminate);

        XEmacPs_BdClear(bdtxterminate);
        XEmacPs_BdSetStatus(bdtxterminate, (XEMACPS_TXBUF_USED_MASK | XEMACPS_TXBUF_WRAP_MASK));
        XEmacPs_WriteReg(xemacpsif->emacps.Config.BaseAddress, XEMACPS_TXQBASE_OFFSET, (kuint32_t)bdtxterminate);
    }
...
//				(Xil_ExceptionHandler)XEmacPs_IntrHandler, (void *)&xemacpsif->emacps);
     ...
     */
//	XScuGic_EnableIntr(XPAR_SCUGIC_0_DIST_BASEADDR, (u32) xtopologyp->scugic_emac_intr);
...
kint32_t XEmacPsIf_SgSend(xemacpsif_s *xemacpsif, struct pbuf *p)
{
    ...
    /*!< Start transmit */
    reg  = XEmacPs_ReadReg(xemacpsif->emacps.Config.BaseAddress, XEMACPS_NWCTRL_OFFSET);
    reg |= XEMACPS_NWCTRL_STARTTX_MASK;
    XEmacPs_WriteReg(xemacpsif->emacps.Config.BaseAddress, XEMACPS_NWCTRL_OFFSET, reg);
...
void XEmacPsIf_SetupRxBds(xemacpsif_s *xemacpsif, XEmacPs_BdRing *rxring)
{
    ...
    if (XEMACPS_IS_ETH_0(&xemacpsif->emacps))
        index = 0;
    ...
    while (freebds > 0) 
    {
        ...
        if (!xemacpsif->emacps.Config.IsCacheCoherent)
            Xil_DCacheInvalidateRange((kuint32_t)p->payload, (kuint32_t)XEMACPS_MAX_FRAME_SIZE);
...
void XEmacPsIf_HandleError(void *args)
{
    ...
    status = XEmacPs_CfgInitialize(&xemacpsif->emacps, sprt_zynq7_machconfig, sprt_zynq7_machconfig->BaseAddress);
    if (status)
...
void XEmacPsIf_ErrorHandler(void *arg, u8 Direction, u32 ErrorWord)
{
    struct xemac_s *xemac;
    xemacpsif_s   *xemacpsif;
    XEmacPs_BdRing *rxring;
...
{
    struct xemac_s *xemac = (struct xemac_s *)args;
    XEmacPs *emacps = &((xemacpsif_s *)xemac->state)->emacps;
...
static struct pbuf *low_level_input(struct netif *netif)
{
    ...
    if (pq_queue_get_size(xemacpsif->recv_q) == 0)
        return NULL;
